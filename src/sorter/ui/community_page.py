...
The catalogue, its filters, and the download →
``import_model(community_download=True)`` path. Layout is the
table-plus-action-bar idiom the Models activity uses, so the two libraries
read alike.
...
* **The page is only usable signed in.** It always exists and shows a sign-in
  panel until there is an account; ``app.py`` hides the sidebar entry until
  then (see ``refresh_auth_state``).
...
def format_size(n: int) -> str:
    """Bytes as the catalogue shows them."""
    if n <= 0:
        return EMPTY_VALUE
    value = float(n)
...
def format_date(iso: str) -> str:
    """Publish date, in the user's OS regional format (``ui.formatting``).

    Thin wrapper kept for the module's existing call site and import; this
    used to hand-roll ``M/D/YYYY`` — see ``ui/formatting.py`` for why that
    hardcoded policy was replaced (JL live-testing).
    """
